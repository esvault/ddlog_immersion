...
double new_c_time(matrix_t *adj) {
    clock_t start = clock();
...
double new_ddlog_time(char **srcs, char **dsts, int edges_num) {
    clock_t start = clock();
...
//    ddlog_dump_table(prog,
//                     (ddlog_get_table_id(prog, "Edges")),
//                     &print_records_callback,
//                     (uintptr_t)(void*)(NULL));
...
//    double t = new_ddlog_time(srcs, dsts, edges_num);
    double t = new_c_time(adj);
    printf("C time: %f\n\n", t);
...
//        printf("C...\n");
//        c_result->data[0][i] = new_c_time(adj);
////        d_result->data[0][i] = ddlog_time(prog, ReachabilityIndexID, 832, 1);
//        printf("DDlog...\n");
//        d_result->data[0][i] = new_ddlog_time(srcs, dsts, edges_num);
//    }
//
